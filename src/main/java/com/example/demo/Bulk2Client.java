...
import request.CloseOrAbortJobRequest;
import request.CreateJobRequest;
import request.GetAllJobsRequest;
import response.CloseOrAbortJobResponse;
import response.CreateJobResponse;
import response.GetAllJobsResponse;
import response.GetJobInfoResponse;
import response.JobInfo;
import type.JobStateEnum;
...
public class Bulk2Client {
    ...
    public CreateJobResponse createJob(String object, OperationEnum operation, Consumer<CreateJobRequest.Builder> requestBuilder) {
        ...
        //★★★★★★更新する場合、外部キーの設定が必須です。★★★★★
        requestBuilder.accept(builder.withExternalIdFieldName("Id"));
